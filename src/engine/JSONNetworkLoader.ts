...
export const loadNetworkFromJSON = (jsonArray: JSONTrackSegment[]) => {
    ...
    jsonArray.forEach((jsonItem) => {
        if (jsonItem.center) {
            // Circular
            const segment = new CircularTrackSegment(jsonItem.start, jsonItem.end, jsonItem.center, jsonItem.counterClockWise);
            ...
            if (jsonItem.stations) {
                jsonItem.stations.forEach((station: JSONStation) => {
                    const stationObject = new Station(segment, station.distanceAlong, station.alignment);
            ...
            // Linear
            const segment = new LinearTrackSegment(jsonItem.start, jsonItem.end);
            segment.id = jsonItem.id;
            createdSegments.set(jsonItem.id, segment);
            network.segments.push(segment);
            if (jsonItem.stations) {
