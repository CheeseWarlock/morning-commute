...
export default function TrackEditorPage() {
    ...
    const [state, setState] = useState<EDITOR_PAGE_STATE>(EDITOR_PAGE_STATE.EDITOR);
    ...
    return (
        <div>
            <TrackEditorTopBar network={network} isNetworkComplete={isNetworkComplete} setNetwork={setNetwork} state={state} setState={setState} />
            {state === EDITOR_PAGE_STATE.EDITOR && (
                <TrackEditorContent network={network} setNetwork={setNetwork} />
            )}
            {state === EDITOR_PAGE_STATE.GAME && (
                <GamePreview network={network} />
            )}
        </div>
    )
}
